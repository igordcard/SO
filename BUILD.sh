...
# so just to be safe
test -h /usr/rift && sudo -HE rm -f /usr/rift

if [[ $PLATFORM == ub16 ]]; then
    # enable the right repos
    curl http://repos.riftio.com/public/xenial-riftware-public-key | sudo -HE apt-key add -
    # the old mkcontainer always enabled release which can be bad
    # so remove it
    sudo -HE rm -f /etc/apt/sources.list.d/release.list /etc/apt/sources.list.d/rbac.list /etc/apt/sources.list.d/OSM.list
    # always use the same file name so that updates will overwrite rather than enable a second repo
    sudo -HE curl -o /etc/apt/sources.list.d/RIFT.list http://buildtracker.riftio.com/repo_file/ub16/${PLATFORM_REPOSITORY}/ 
    sudo -HE apt-get update
        
    # and install the tools
    sudo -HE apt remove -y rw.toolchain-rwbase tcpdump
    sudo -HE apt-get install -y --allow-downgrades rw.tools-container-tools=${PLATFORM_VERSION} rw.tools-scripts=${PLATFORM_VERSION} python 
elif [[ $PLATFORM == fc20 ]]; then
    # get the container tools from the correct repository
    sudo -HE rm -f /etc/yum.repos.d/private.repo
    sudo -HE curl -o /etc/yum.repos.d/${PLATFORM_REPOSITORY}.repo \
	 http://buildtracker.riftio.com/repo_file/fc20/${PLATFORM_REPOSITORY}/ 
    sudo -HE yum install --assumeyes rw.tools-container-tools rw.tools-scripts
else
    echo "Internal error: unknown platform $PLATFORM"
    exit 1
fi
...
if $runMkcontainer; then
    sudo -HE apt-get install -y libxml2-dev libxslt-dev
    sudo -HE /usr/rift/container_tools/mkcontainer --modes build --modes ext --repo ${PLATFORM_REPOSITORY}
    sudo -HE pip3 install lxml==3.4.0
fi
...
if [[ $PLATFORM == ub16 ]]; then
    ...
    for package in $PACKAGES $OLD_PACKAGES; do
        sudo -HE apt remove -y $package || true
    done
    ...
    sudo -HE apt-get install -y --allow-downgrades $packages

    sudo -HE apt-get install python-cinderclient
    
    sudo -HE chmod 777 /usr/rift /usr/rift/usr/share
    
    if $installSO; then
	sudo -HE apt-get install -y \
	...
	if rpm -q $(rpm -q -p $pkg) >/dev/null; then
	    ...
	    sudo -HE rpm -i --replacefiles --nodeps $pkg
	fi
    done
    ...
    sudo -HE rm -f /usr/rift/usr/lib/libmano_yang_gen.so
...
if [[ $UIPathToBuild ]]; then
    ...
    sudo -HE make -C $UIPathToBuild install
fi

echo "Creating Service ...."
sudo -HE $(dirname $0)/create_launchpad_service
